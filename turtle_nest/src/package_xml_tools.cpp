...
bool PackageXMLEditor::has_dependency(QString dependency, DependencyType depend_type){
    ...
    for (XMLElement* dependElem = root->FirstChildElement(depend_cstr); dependElem != nullptr; dependElem = dependElem->NextSiblingElement(depend_cstr)) {
...
XMLElement* PackageXMLEditor::find_dep_insert_point(std::string depend_str){
    ...
    for (XMLElement* dep = root->FirstChildElement(depend_str.c_str()); dep; dep = dep->NextSiblingElement(depend_str.c_str())) {
        insert_after = dep;
    }
    ...
    if (!insert_after){
        throw std::runtime_error("Failed to insert a new dependency tag in to the package.xml. Update package.xml manually!");
    }
    return insert_after;
}
