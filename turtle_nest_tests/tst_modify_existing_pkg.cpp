...
void execute_node_and_assert_success(PackageInfo pkg_info, QString node_name, NodeType node_type){
    ...
    QString expected_output = QString("[%1]: Hello world from the %2 node %1").arg(node_name, node_type_str);
...
TEST(modify_existing_pkg, add_node_to_existing_package){
    ...
    for (TestCase &test_case : test_cases) {
        ...
        RosPkgCreator pkg_creator(pkg_info.workspace_path, pkg_info.package_name, pkg_info.package_type);
...
TEST(modify_existing_pkg, add_node_xml_tests){
    ...
    QList<TestCase> test_cases = {
        ...
        {CPP, CPP_NODE, "rclcpp_build_and_exec_depend.package.xml", "rclcpp_build_and_exec_depend.package.xml"}, // Expect no changes
    ...
    for (const auto & test_case : test_cases){
        ...
        QString expected_xml_path = QString(QDir(fixtures_folder).filePath(test_case.expected_result_xml_file));
...
TEST(modify_existing_pkg, DISABLED_manual_integration_test_add_new_node){
    ...
    QStringList ignored_packages = {"nav2_common", "karto_sdk", "rosbridge_suite", "teleop_twist_keyboard"};
    ...
    for (auto & package : packages){
        ...
        for (int i = 0; i < node_names.size(); ++i) {
            ...
            QString expected_output = QString("[%1]: Hello world from the %2 node %1").arg(node_name, print_string);
            QString output = run_command(
                QString("ros2 run %1 %2").arg(pkg_info.package_name, node_name),
                {expected_output},
